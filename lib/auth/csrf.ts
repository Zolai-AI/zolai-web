 ...
 * Skips Better Auth routes (they have their own CSRF protection)
 * Skips authenticated requests (session cookie provides CSRF protection via SameSite)
 */
export const csrfMiddleware = async (c: Context, next: Next) => {
  // Skip Better Auth routes (they handle CSRF internally)
  const path = new URL(c.req.url).pathname;
  if (path.startsWith("/api/auth/")) {
    return next();
  }

  // Skip local AI chat proxy endpoints (they do not mutate server state)
  if (path === "/api/zolai/chat" || path === "/api/zolai/chat/stream") {
    return next();
  }
  ...
  // Skip for authenticated requests (session cookie provides CSRF protection)
  const sessionCookie = c.req.header("cookie");
  if (sessionCookie?.includes("better-auth.session_token") || sessionCookie?.includes("__Secure-better-auth.session_token")) {
    return next();
  }
