...
// Fallback to env var; DB setting checked at upload time
const _DEFAULT_MAX_FILE_SIZE = parseInt(process.env.MAX_UPLOAD_SIZE_MB || "5") * 1024 * 1024;
...
export async function uploadFile(
  ...
  // Read limits from DB settings at upload time (no restart needed)
  const [maxMbStr, allowedTypesStr] = await Promise.all([
    getSiteSetting("max_upload_size", "5"),
    getSiteSetting("allowed_file_types", "image/jpeg,image/png,image/gif,application/pdf"),
  ]);
  const maxFileSize = (parseInt(maxMbStr) || 5) * 1024 * 1024;
  const allowedTypes = allowedTypesStr.split(",").map((t) => t.trim()).filter(Boolean);

  if (options.size > maxFileSize) {
    throw new Error(`File size exceeds maximum allowed size of ${maxMbStr}MB`);
  }
  if (allowedTypes.length && !allowedTypes.includes(options.mimeType)) {
